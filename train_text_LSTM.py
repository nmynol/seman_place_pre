...
def data_prepare(config):
    ...
    # return load_text(tr_text, "train"), load_text(tr_text, "val"), tr_y, val_y
...
def main():
    ...
    val_dataset = ImageFolder(val_text_emb, val_y)
    ...
    for epoch in range(last_epoch, config.epoch - 1):
        ...
        for iter, [text, y] in enumerate(train_dataloader):
            ...
            err_text.backward()
            optimizerT.step()
            iter_right, iter_total, _ = cal_acc(pred_text.cpu().detach(), y.cpu().detach())
        ...
        if (epoch + 1) % config.val_freq == 0:
            val(val_dataloader, netT.eval(), device)
...
def val(val_dataloader, netT, device):
    right_val = 0
    total_val = 0
    for iter, [val_text, val_y] in enumerate(val_dataloader):
        val_text = val_text.to(device)

        with torch.no_grad():
            pred_val_text = netT(val_text)

        iter_right_val, iter_total_val, _ = cal_acc(pred_val_text.cpu(), val_y)
        right_val = right_val + iter_right_val
        total_val = total_val + iter_total_val

    print("text accuracy   | test: %.3f %% " % ((right_val / total_val) * 100))
