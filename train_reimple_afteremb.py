...
import data_helpers
from models.origin_model import CNN_fusion_emb
...
def data_prepare(config, mode):
    ...
    #             (0, max(train_text.shape[1], test_text.shape[1]) - min(train_text.shape[1], test_text.shape[1])),
    #             (0, 0)), 'constant', constant_values=((0, 0), (0, 0), (0, 0)))
    #     text = np.concatenate((train_text, test_text), axis=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_mode', default='total', type=str)
    parser.add_argument('--model_mode', default='simple', type=str)
    ...
    netF = torch.nn.DataParallel(CNN_fusion_emb(args.model_mode))
    ...
    tr_text, test_text, tr_image, test_image, \
    tr_user, test_user, tr_y, test_y = data_prepare(config, args.data_mode)

    train_dataset = ImageFolder(tr_text, tr_image, tr_user, tr_y)
    ...
    for epoch in range(last_epoch, config.epoch - 1):
        for iter, [text, image, user, y] in enumerate(train_dataloader):
            netF.train()
            netF.zero_grad()
            image, user, y = image.to(device), user.to(device), y.to(device)
        ...
        if (epoch + 1) % config.val_freq == 0:
            val(tr_text, test_text, tr_image, test_image, tr_user, test_user, tr_y, test_y, netF.eval(), device,
                epoch, writer)
