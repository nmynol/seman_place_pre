...
def cal_acc(pred_text, y):
    ...
    return len(per_text[per_text == True]), len(per_text), text_acc
    # return text_acc
...
def main():
    ...
    val_dataset = ImageFolder(val_text_emb, val_image, val_user, val_y)
    val_dataloader = data.DataLoader(val_dataset, batch_size=config.batch_size, drop_last=True,
                                     shuffle=True, pin_memory=True, num_workers=int(config.workers))
    ...
    for epoch in range(last_epoch, config.epoch - 1):
        ...
        for iter, [text, image, user, y] in enumerate(train_dataloader):
            ...
            pred_fusion = netF(text, image, user)
            err_text = criterion(pred_fusion, y.argmax(dim=1))
            # print("err_text: ", err_text)
            err_text.backward()
            optimizerF.step()
            iter_right, iter_total, _ = cal_acc(pred_fusion.cpu().detach(), y.cpu().detach())
            right = right + iter_right
            total = total + iter_total

        print("epoch" + str(epoch) + ": train accuracy: %.3f %%" % ((right/total) * 100))

        if (epoch + 1) % config.val_freq == 0:
            val(val_dataloader, netF.eval(), device)
            # val(tr_text_emb, val_text_emb, tr_y, val_y, netF.eval(), device)
...
def val(val_dataloader, netF, device):
    right_val = 0
    total_val = 0
    for iter, [val_text, val_image, val_user, val_y] in enumerate(val_dataloader):
        val_text, val_image, val_user = val_text.to(device), val_image.to(device), val_user.to(device)

        with torch.no_grad():
            pred_val_fusion = netF(val_text, val_image, val_user)

        iter_right_val, iter_total_val, _ = cal_acc(pred_val_fusion.cpu(), val_y)
        right_val = right_val + iter_right_val
        total_val = total_val + iter_total_val

    print("fusion accuracy   | test: %.3f %% " % ((right_val / total_val) * 100))
...
#
#     print("fusion accuracy   | train: %.3f %% | test: %.3f %% " % (tr_text_acc, val_text_acc))
