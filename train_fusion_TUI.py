...
import data_helpers
from models.model_CNN import TUI, TUI_img_user
...
def main():
    ...
    parser1 = argparse.ArgumentParser(description='PyTorch for image-user CNN')
    # parser.add_argument('--embed_num', default=)
    parser1.add_argument('--embed_dim', default=300)
    parser1.add_argument('--class_num', default=8)
    parser1.add_argument('--kernel_num', default=100)
    parser1.add_argument('--kernel_sizes', default=[1])
    # parser1.add_argument('--max_norm', default=)
    # parser1.add_argument('--paddingId', default=)
    # parser1.add_argument('--word_Embedding', default=)
    # parser1.add_argument('--pretrained_weight', default=)
    parser1.add_argument('--wide_conv', default=True)
    parser1.add_argument('--init_weight', default=True)
    parser1.add_argument('--init_weight_value', default=2.0)
    parser1.add_argument('--cuda', default=True)
    parser1.add_argument('--dropout', default=0.75)
    parser1.add_argument('--dropout_embed', default=0.5)
    parser1.add_argument('--batch_normalizations', default=False)
    parser1.add_argument('--batch_norm_affine', default=False)
    ...
    netF = torch.nn.DataParallel(TUI_img_user(config1, 4))
...
def val(tr_text_emb, val_text_emb, tr_image, val_image, tr_user, val_user, tr_y, val_y, netF, device, epoch, writer):
    ...
    with torch.no_grad():
        pred_tr = netF(tr_user, tr_image, tr_text)
        pred_val = netF(val_user, val_image, val_text)
