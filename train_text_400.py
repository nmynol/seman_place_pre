...
def load_text(x_text, name):
    ...
    model = gensim.models.KeyedVectors.load_word2vec_format('./alldata/vectors400.txt', binary=False)
...
def main():
    ...
    for epoch in range(last_epoch, config.epoch - 1):
        ...
        if (epoch + 1) % config.val_freq == 0:
            val(tr_text_emb, val_text_emb, tr_y, val_y, netT.eval(), device, epoch)
...
def val(tr_text_emb, val_text_emb, tr_y, val_y, netT, device, epoch):
    ...
    print("epoch " + str(epoch) + " text accuracy   | train: %.3f %% | test: %.3f %% " % (tr_text_acc, val_text_acc))
